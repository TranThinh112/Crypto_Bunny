...
from crypto_trader.lc_pipeline import lc_pipeline_mini_pool, notify_mini_pool_summary, update_lc_internal_pipeline
from crypto_trader.models import TradeCandidate
from crypto_trader.storage import get_journal_state


def _candidate(symbol: str, win: float, confidence: float = 80.0, volume: float = 1.0) -> TradeCandidate:
...
class LcPipelineTest(TestCase):
    ...
    def test_two_hour_summary_keeps_top_three_and_notifies(self, send_message) -> None:
        ...
        self.assertIn("LC nội bộ tổng hợp 2h #1", send_message.call_args.args[1])
